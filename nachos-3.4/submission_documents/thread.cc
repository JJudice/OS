...
//BEGIN changes by JJT
	printf("Thread %d is being terminated \n", SpaceID);
//END changes by JJT
	//delete space;
...
// increment and set the space ID for a new thread
//BEGIN changes JJT	
	incID();
	this->SpaceID = getID();
	answer = this->SpaceID;
	DEBUG('t', "Forking thread \"%s\" with func = 0x%x, arg = %d SpaceID = %d\n",
	  name, (int) func, arg,answer );
...
//-----------------------------------------------------------
// Thread::set ID
// this will be the unique identifier used to tell each 
// thread apart
//-----------------------------------------------------------
...
void
Thread::Join(int parInt, int kidInt)
{
    parent_id = parInt;
    child_id = kidInt;
    Thread *nextThread;
    ...
    DEBUG('t', "Parent Thread Yielding \"%s\"\n", parent_id);
    
    while ((CThread = scheduler->FindNextToRun()) == NULL) 
    {
	scheduler->ReadyToRun(this);
	scheduler->Run(CThread);
    }
    (void) interrupt->SetLevel(oldLevel);
