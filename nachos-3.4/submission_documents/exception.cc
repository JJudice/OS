...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		case SC_Join :
			...
			space2 = new AddrSpace(ChildExec);;
			...
			break;	
				//Not done as of now but maybe close
/*
			answer = currentThread->getSpaceID();
			child_id = answer +1;
			parent_id = answer;
			...
			if ((CurrentExec == NULL)||(ChildExec == NULL)) 
			{
				printf("Unable to open file %s\n", ch3);
				return;
			}
			...
			space3 = new AddrSpace(CurrentExec);
			parent = new Thread("parent-worker");
			parent->space = space3;
			answer2 = getID();	
			//parent_id = getID();
			...
			// Might make a function to get parent and child data in thread.cc
			
			
			delete CurrentExec;			// close file
			delete ChildExec;			// close file
			//currentThread->Finish();
			
			DEBUG('t',"leaving Join \n");	
			break;	

			// this is the commented version of join that is 
			// not complete for submission.
			// It is here to show that it was worked on and 
			// effert was made to complete it
			// t_int = currentThread->getSpaceID();
			// printf("SYSTEM CALL:Join called by thread %d\n",t_int);
			//put currentThread to sleep
			// old_Thread = currentThread;
			// DEBUG('t',"inside Join \n");
			// DEBUG('t',"type = %d arg1 = %d, arg2 =%d, arg3 =%d \n",type, arg1,arg2, arg3);
			
			// for (int jj = 0;jj < arg1 ; jj++) {
				// if(!machine->ReadMem((arg1+jj), 1, &ii))
					// jj=jj-1;
				// else{
					// ch2[jj] = (char) ii;
			...
			//	creat a new thread 
				// t = new Thread("Join-worker");
				// t->space = space2;

				// DEBUG('t',"line 213 right before fork\n");
				// answer2 = getID();

				// t->Fork(CreateJoinThread,answer2);
